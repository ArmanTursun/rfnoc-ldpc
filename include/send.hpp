...
#include </root/rfnoc_test.ldpc/include/rfnoc/example/polar_encoder_rfnoc.hpp>


///////////////////////////////////////////////////////////////////////////////

uint64_t swap_iq_(unsigned i, std::vector<uint64_t> buffer){
...
void send_one_packets(){
    ...
    for (unsigned it = 0; it < tx_buffer_1.size(); ++it){
    	tx_buffer_1[it] = swap_iq_(it, tx_buffer_1);
    ...
    for (unsigned it = 0; it < tx_buffer_2.size(); ++it){
    	tx_buffer_2[it] = swap_iq_(it, tx_buffer_2);
    ...
    for (unsigned it = 0; it < tx_buffer_3.size(); ++it){
    	tx_buffer_3[it] = swap_iq_(it, tx_buffer_3);
    ...
    for (int i = 0; i < num_requested_samples_1 / 2; i++){
    	rx_buf_ptr_1[i] = swap_iq_(i, rx_buf_ptr_1);
    ...
    for (int i = 0; i < num_requested_samples_2 / 2; i++){
    	rx_buf_ptr_2[i] = swap_iq_(i, rx_buf_ptr_2);
    ...
    for (int i = 0; i < num_requested_samples_3 / 2; i++){
    	rx_buf_ptr_3[i] = swap_iq_(i, rx_buf_ptr_3);
